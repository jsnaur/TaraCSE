...
import { Check, X, Sparkles, ArrowRight, ArrowLeft } from "lucide-react";
  ...
  return (
    <div className="flex h-screen w-full bg-background text-foreground overflow-hidden font-sans transition-colors duration-200">
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Topbar */}
        <div className="px-5 h-[52px] min-h-[52px] border-b border-border flex items-center gap-3 bg-background transition-colors duration-200">
          <div className="font-heading text-[15px] font-bold text-foreground tracking-tight">
            Dashboard
          </div>
          <div className="flex items-center gap-1.5 bg-card border border-border rounded-full px-3 py-1 text-xs text-primary font-semibold">
            <div className="w-2 h-2 rounded-full bg-accent"></div>
            12-day streak
          </div>
          <div className="ml-auto flex items-center gap-4 text-[11px] text-muted-foreground">
            <span>Saturday, Apr 18 &nbsp;&middot;&nbsp; CSE Exam in 47 days</span>
            <ThemeToggle />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-4">
          ...
          {/* Hero Card */}
          <div className="bg-card border border-border rounded-xl p-5 grid grid-cols-[1fr_auto] gap-4 items-center">
            <div>
              ...
              <div className="text-xs text-muted-foreground leading-relaxed max-w-[460px]">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div>
                ...
                <div className="flex flex-col gap-1.5">
                  <div className="bg-card border border-border rounded-lg p-2.5 flex items-center gap-2.5">
                  ...
                  <div className="bg-card border border-border rounded-lg p-2.5 flex items-center gap-2.5">
                    ...
                    <div className="text-xs text-muted-foreground flex-1 leading-snug">Flashcard session &mdash; Philippine History (40 cards)</div>
          ...
          <div className="bg-card border border-border rounded-xl overflow-hidden shadow-sm">
            {/* Exam Header */}
            <div className="px-4 py-3 border-b border-border flex items-center gap-2.5 bg-sidebar">
              <div className="text-[9px] font-bold tracking-[0.1em] uppercase bg-primary/10 text-primary px-2.5 py-1 rounded-full">
                Analytical Ability
              </div>
              <div className="text-[11px] text-muted-foreground">Question 3 of 40</div>
            ...
            <div className="p-4">
              ...
              <div className="text-sm leading-relaxed font-medium mb-3.5">
              ...
              <div className="flex flex-col gap-1.5">
                {[
                  ...
                  if (showCorrect) {
                    optClass = "bg-[var(--spark-correct-bg)] border-[var(--spark-correct-border)] text-[var(--spark-correct-text)]";
                    checkIcon = <Check className="w-4 h-4 ml-auto font-bold" />;
                  } else if (isWrongSelected) {
                    optClass = "bg-[var(--spark-wrong-bg)] border-[var(--spark-wrong-border)] text-[var(--spark-wrong-text)]";
                    checkIcon = <X className="w-4 h-4 ml-auto font-bold" />;
                  }
                  ...
                  return (
                    <button 
                      ...
                      className={`flex items-center gap-2.5 px-3.5 py-2.5 rounded-md border text-[13px] text-muted-foreground transition-all text-left ${optClass} ${!selectedOption && "hover:text-foreground"}`}
                    >
                      <span className={`font-heading text-[11px] font-extrabold min-w-[16px] ${showCorrect ? "text-[var(--spark-correct-text)]" : isWrongSelected ? "text-[var(--spark-wrong-text)]" : "text-muted-foreground"}`}>
                        {opt.ltr}
                      </span>
                      {opt.text}
                      {checkIcon}
            ...
            {/* AI Block */}
            {selectedOption && (
              <div className="mx-4 mb-4 bg-[var(--spark-ai-bg)] border border-[var(--spark-ai-border)] rounded-md px-3.5 py-3">
                <div className="flex items-center gap-2 mb-2">
                  <div className="w-4.5 h-4.5 bg-primary/20 border border-[var(--spark-ai-border)] rounded flex items-center justify-center shrink-0">
                    <Sparkles className="w-2.5 h-2.5 text-[var(--spark-ai-text)]" />
                  </div>
            ...
            <div className="px-4 py-3 border-t border-border flex items-center gap-2">
              <button className="px-4 py-2 rounded-md font-sans text-xs font-semibold text-muted-foreground hover:bg-sidebar hover:text-foreground transition-colors flex items-center gap-1.5">
                <ArrowLeft className="w-3.5 h-3.5" /> Back
              </button>
              <button className="px-4 py-2 rounded-md font-sans text-xs font-semibold bg-[var(--spark-ai-bg)] border border-[var(--spark-ai-border)] text-[var(--spark-ai-text)] hover:opacity-85 transition-opacity flex items-center gap-1.5">
                <Sparkles className="w-3.5 h-3.5" /> Hint
              </button>
              <button 
                className="ml-auto px-4 py-2 rounded-md font-sans text-xs font-semibold bg-primary text-primary-foreground hover:opacity-90 transition-opacity flex items-center gap-1.5"
